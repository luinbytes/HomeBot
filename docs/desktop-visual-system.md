...
The native executable uses eframe 0.32.3 with accessibility, OpenGL, Wayland and X11 support. Live builds prefer the host's system sans-serif font—San Francisco on macOS and Noto Sans or DejaVu Sans on Linux—while deterministic goldens retain the embedded test font. Its Windows clipboard transitive dependencies use the OSI-approved permissive Boost Software License 1.0, which is admitted explicitly by the dependency policy.
...
The ordinary desktop hierarchy is a responsive two-pane shell: an identity-led sidebar occupies 30% of the reference window (bounded from 276 to 324 logical points), while chat remains the dominant surface. Search, chat, Routines and Assistant Packs remain primary destinations. Settings, Bot editing, destructive confirmation, Assistant Pack configuration, routine editing/teaching, group coordination and computer/coding details use one focused modal layer with a dimmed backdrop, a single clear exit and viewport-bounded scrolling. The bottom composer is a persistent rounded action surface; queue, steering and stop are disclosed only while a Bot is running.

Primary actions use the semantic accent fill, destructive actions use the semantic danger fill, and secondary actions remain neutral. Text fields have a distinct input surface and focus stroke in both themes. Motion is limited to the sidebar and state transitions, is reversible, and becomes immediate when Reduce interface motion is enabled.
...
These images exercise the production sidebar, header, transcript, activity and approval cards, anchored composer, settings and device-security navigation, Bot lifecycle dialogs, Assistant Pack configuration, routine editor/recording workflows, group coordination, contextual details, errors and destructive confirmation. The legacy showcase remains a component development aid, not release evidence.
