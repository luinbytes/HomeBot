...
The HomeBot release baseline now has deterministic CPU-rendered goldens for the real production `HomeBotApp` shell: populated direct chat and composer, activity, approval, group chat, disconnected/provider-unavailable, settings, routine list/editor/recording and contextual computer/details. These cover production regression stability but remain `Capture required` except where legitimate comparison evidence is explicitly recorded; see [desktop-visual-system.md](desktop-visual-system.md).
