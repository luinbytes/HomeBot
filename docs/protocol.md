...
Pairing is owner-initiated but exchange is the sole narrowly unauthenticated mutation. `POST /api/v1/pairing` returns a five-minute single-use offer whose identifier, endpoint and credential are bound together; `POST /api/v1/pairing/exchange` must echo that offer identity and endpoint, atomically consumes it, and returns a named persistent device session once. Owner-only device routes list/revoke sessions. Raw tokens and client addresses never enter SQLite or events: exchange throttling stores digests in source-scoped buckets. Device bearers authenticate the same HTTP/WebSocket protocol but cannot administer devices. See [remote-access.md](remote-access.md).
...
Browser session projections include the current controller and, during human takeover, the controlling device and lease expiry. The server changes those fields atomically: a second device cannot replace a live lease, issue actions, or return control. Browser profile identifiers select isolated server-side CDP contexts; clients never select or receive raw browser context IDs, cookie stores, or profile paths.
