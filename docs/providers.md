...
When a Bot has HomeBot tools, the adapter starts an operation-scoped MCP server on an unguessable loopback URL and passes its configuration to Claude with `--mcp-config`. Only that turn's tools are listed and allowlisted. A Claude MCP call becomes the same structured `ProviderEvent::ToolCall` used by Codex, OpenAI-compatible APIs, and generic process adapters; HomeBot applies its normal Bot/plugin/policy boundary and returns the result to the waiting MCP request. Calls, listeners, and pending results are removed when the operation completes or is cancelled. No HomeBot bearer token, plugin credential, or raw result is placed in the Claude prompt or process environment.
...
Responses API profiles support streamed text, reasoning/tool activity, usage, cancellation and `previous_response_id` continuation. Chat Completions profiles support streamed text, usage, cancellation and same-turn tool continuation; provider-native resume remains unavailable until the server supplies full cross-turn transcript replay. Both styles pass the Bot's assigned tools as native function definitions, route structured calls through the server's normal policy/plugin/Bot boundary, and continue only after a bounded structured result returns. A model response may request at most 32 HomeBot tools, repeated call IDs and malformed streamed arguments fail closed, and cancellation remains active while a result is pending. Model discovery uses `GET /models`. Remote endpoints require HTTPS, while explicit loopback endpoints may use HTTP for local Ollama, LM Studio and similar servers. Redirects are disabled so bearer credentials cannot be forwarded unexpectedly.
...
Resume uses `kind: "resume"` and includes `conversation_id`. The child writes one serialized `ProviderEvent` per stdout line and must finish with exactly one `completed`, `cancelled` or `failed` event. A child may emit a structured `tool_call`; HomeBot routes it through the normal policy and plugin boundary, then writes a `kind: "tool_result"` record with the same call ID back to the still-running child. Pending calls are operation-scoped, duplicate IDs fail closed, and the advertised tool definitions are included in the start/resume record. Lines and event queues are bounded; malformed output fails closed; cancellation closes stdin and then enforces the supervisor deadline. Arguments and environment values are omitted from debug output. Community adapters that need a richer native protocol should implement `ProviderAdapter` directly instead of adding provider-specific fields to this contract.

## First-party memory providers

The server owns memory connections and credentials; Android only configures and assigns them. The catalog covers Supermemory, Honcho, Hindsight, Holographic, Mem0/OpenMemory, Zep Cloud, Graphiti, Cognee, Letta, and LangMem/LangGraph Store, while keeping unavailable REST, OAuth, and framework bridges visibly non-active instead of pretending a catalog row is support. Poncho is an agent harness rather than a memory provider and is deliberately not presented as one.

Automatic scoped recall/retain is active for hosted Supermemory MCP, self-hosted Supermemory REST, hosted or self-hosted Honcho MCP, hosted or self-hosted Hindsight MCP, hosted Mem0 MCP, self-hosted OpenMemory REST, self-hosted Graphiti MCP, and Cognee MCP bridges. HomeBot isolates data by owner and Bot using each provider's native scope: Supermemory container tags, Honcho workspace/peer/session IDs, Hindsight tags, Mem0/OpenMemory user and agent/app filters, Graphiti group IDs, and Cognee datasets. The self-hosted Supermemory adapter uses its documented `/v3/search` and `/v3/documents` API. The OpenMemory adapter uses the current Mem0 server's `/search` and `/memories` endpoints and optional `X-API-Key` authentication. Both keep credentials in the server vault, expose only scoped recall/retain operations, and bound untrusted responses. Recall output is injected as explicitly untrusted context; provider outages never block the Bot turn. Honcho lazily creates the stable user/Bot peers and conversation session before retaining the exact latest exchange.

Holographic runs as a credential-free built-in provider backed by the HomeBot database. Its `fact_store` tool supports explicit add, search, probe, related, reason, update, remove, and list operations; `fact_feedback` applies the upstream helpful/unhelpful trust adjustments. Facts, FTS search, entities, trust, and deletion are isolated by owner and Bot and survive restart. Assigned facts are prefetched for the next turn, but HomeBot does not silently convert transcripts into facts. Optional HRR contradiction scoring is not implemented and is reported as unavailable rather than approximated.

Zep Cloud/BYOC is now connectable through the generic server-owned MCP OAuth/PKCE path when its authorization server permits dynamic client registration; live Zep consent and graph-scope acceptance are still unproven. Letta Archives and LangMem/LangGraph bridges remain catalogued but unavailable until their distinct lifecycle contracts are implemented. See [memory-provider-research.md](memory-provider-research.md) for primary-source contracts and the remaining acceptance work.
