...
| Stolen pairing link | Short expiry, single use, hashed token, explicit offer/endpoint binding, source-scoped and per-offer rate limits, revocation | Mismatched offer provenance, second exchange and post-expiry exchange fail; one source cannot exhaust another source's bucket |
...
Desktop update manifests are Ed25519 signed over a canonical schema and verified against a public key pinned into the release build before any artifact metadata is trusted. Artifact SHA-256 and byte length are checked again while staging. The release private key remains offline; builds without an explicitly supplied public key fail closed and cannot silently use a fixture key.
...
The browser server API exposes only opaque profile/session IDs, display metadata, normalized current URLs, controller/status state, and content-addressed screenshot artifacts. Profile directory references are generated by the server as a single path component, while each profile is mapped to a distinct CDP browser context and every returned target is verified as a member of that context before use. Cookies are therefore separated between HomeBot profiles and never cross the client protocol. Group ownership handoff does not copy or reveal browser state: subsequent Bots consume the same server-held session projection. Takeover, navigation, close, and other sensitive transitions pass through the capability engine, persist an awaiting-approval state, and resume only after the normal exact approval decision. A successful takeover creates a durable, expiring controller lease bound to the authenticated device; competing devices cannot act, return control, or replace that live lease.
