...
Authenticated global search is owner-scoped and returns exact immutable `homebot://` navigation targets for messages, uploaded files, generated artifacts, links and routines. The response carries `ready` or `unavailable`; an unavailable FTS index is not rendered as an empty successful search. Clients treat search and timeline results as replaceable server projections and do not rebuild an independent index.
