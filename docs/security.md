...
The browser server API exposes only opaque profile/session IDs, display metadata, normalized current URLs, controller/status state, and content-addressed screenshot artifacts. Profile directory references are generated by the server as a single path component; cookies and login state remain exclusively server-side. Each profile maps to a distinct CDP browser context, and every action revalidates target membership before connecting. The profile root contains HomeBot metadata and is not passed to the browser as a user-data directory; browser-context continuity ends if the external browser process restarts, so HomeBot does not claim disk-persistent browser credentials. Group ownership handoff does not copy or reveal browser state: subsequent Bots consume the same server-held session projection. Takeover, navigation, close, and other sensitive transitions pass through the capability engine, persist an awaiting-approval state, and resume only after the normal exact approval decision. Human takeover also persists the authenticated controlling device and a bounded lease; Bot/owner actions and competing devices fail before CDP execution until that device returns control.
